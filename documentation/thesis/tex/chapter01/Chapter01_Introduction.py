...
#
# For this study, disease genome-wide association study (GWAS) summary results and published drug-gene interaction data sets were curated and used as input to gene set enrichment analyses to seek robust genetic evidence that can identify drug repurposing opportunities for several complex neuropsychiatric and neurodegenerative diseases - schizophrenia, major depressive disorder, bipolar disorder, Alzheimer's disease, Parkinson's disease and Huntington's disease. Prior to this activity, this study analyses better understood medical conditions, hypertension and hypercholesterolaemia, to initially validate the input data sets and approaches used.
#
...
#
# A MAGMA gene analysis uses as a default setting an adaptive permutation method to correct output p-values for multiple testing, where the number of permutations calculated is dependent on the gene association p-value. This adaptive method, used to minimise compute resource use, stops once a predefined number of permutations, 10 by default, are calculated with a test statistic more extreme than that found for the observed data giving the dependence on the p-value, as lower p-values would on average require more permutations to meet the criteria. The number of permutations matching the criteria is checked periodically by MAGMA after a set number of permutation calculations.
#
# The adaptive method was found to give a huge variation in output p-values over multiple runs with the same input data, partly due to the inherent randomness of permutation calculations but this also would be exacerbated by the batching mechanism adding to the variation. Instead a fixed number of permutations was found to be more reliable at an acceptable level of compute resource use - a value of 20,000 permutations was chosen as the most pragmatic to ensure reliability of the results, taking of the order of 12 hours to run on the available compute infrastructure. See Appendix \ref{appendix-magma-permutation-issues} for further details.
