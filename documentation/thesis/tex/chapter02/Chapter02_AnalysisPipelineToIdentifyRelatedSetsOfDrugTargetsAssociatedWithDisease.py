...
# ## Introduction
#
# This part of the study describes the curation of a drug-target interaction database incorporating the most comprehensive interaction coverage available, used as an input to the gene-set analysis described in the previous chapter, and explores the improvement and usefulness of the curated database over those already existing. The resulting database has been named DUGGIE, an abbreviation of 'DrUG-Gene IntEractions'. 
#
...
#
# UniProt identifiers, which often are used to record the targets of drugs also have an official protocol for mapping to Entrez Gene IDs<citet data-cite="noauthor_mapping_2019"><sup>mapping</sup></citet>. While specific isoforms of a protein, for example, can also be mapped individually to Ensembl IDs from UniProt IDs, their use in conversions between datasets can lead to data being discarded unnecessarily unless the relationship between Ensembl IDs is factored into the process.
