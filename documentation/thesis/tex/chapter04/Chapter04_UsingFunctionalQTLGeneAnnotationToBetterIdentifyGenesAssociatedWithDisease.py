...
# ### SNP-to-gene mapping methods
#
# A variety of SNP-to-gene mappings have been employed in studies to date with a more recent comparison analysis by Gazal et al.<citet data-cite="gazal_combining_2022"><sup>gazal</sup></citet>, subsequent to this work, providing a useful description of available mapping datasets and strategies, including combinations of those strategies. The Gazal et al. analysis further introduced a framework for evaluating SNP-to-gene strategies by measuring the coverage and precision of the heritability explained by each mapping. The analysis constructed linear weighted combinations of the strategies to investigate an optimal combined SNP-to-gene strategy. 
#
# The Gazal et al. study comparison included a range of proximity mappings, such as allocating SNPs to nearest exons, SNPs to transcription start sites (TSS) or SNPs to gene bodies, all with and without flanking regions. Gene expression and epigenetic mappings were also compared, including GTEx cis-eQTL variant-gene associations, fine mapped GTEX cis-eQTL mappings, Roadmap enhancer-gene links<citet data-cite="roadmap_epigenomics_consortium_integrative_2015"><sup>roadmap</sup></citet>, EpiMap<citet data-cite="boix_regulatory_2021"><sup>boix</sup></citet> enhancer-gene links and Activity-by-Contact (ABC) links<citet data-cite="nasser_genome-wide_2021"><sup>nasser</sup></citet>. The SMP-to-gene mapping dataset from OpenTargets<citet data-cite="mountjoy_open_2021"><sup>mountjoy</sup></citet> was included as an additional SNP-to-gene strategy. The OpenTargets dataset was constructed using a machine-learning model trained on fine-mapped genetics data, functional genomics data and curated GWAS loci.
...
#
# Tissue specific expression QTL data from GTEx analysis version 8, obtained from the GTEx portal <citet data-cite="noauthor_gtex_nodate"><sup>gtex</sup></citet> with dbGap accession number phs000424.v8.p2 were used as a source of functional SNP-gene associations. Data from 48 non-diseased tissue types were used. The GTEx data was generated<citet data-cite="noauthor_gtex_nodate-1"><sup>gtex_method</sup></citet> using genotype data based on whole genome sequencing of 838 donor samples that also have RNA-seq data available. With a cis-eQTL defined by GTEx as an eQTL within 1Mb of a gene transcription start site (TSS), cis-eQTL mapping was performed by GTEx using the FastQTL package using adaptive permutations and a false discovery rate threshold of 0.05 or less. 
#
# ##### Methylation QTL
#
# Genome-wide methylation profiles from the substantia nigra and the frontal cortex of 134 individuals with Parkinson's disease were obtained<citet data-cite="kia_identification_2021"><sup>pd_mqtl</sup></citet>. cis-methylation QTLs, defined as correlations between a SNP and DNA methylation levels of a CpG site within a 500kb window of the SNP. The data as obtained from Kia et al.<citet data-cite="kia_identification_2021"><sup>pd_mqtl</sup></citet> included five covariates and the strongest SNP-CpG pairs were retained at a false discovery threshold of 0.05 or less, where CpG sites were mapped to genes if they were within 10kb of a gene TSS or end base position - Kia et al. found 37,460 CpG sites eligible for inclusion in this dataset. Only the frontal cortex data was used in this study, as substantia nigra tissue is likely to be modified from Parkinson's disease.
#
# ##### PsychENCODE QTL
#
# The PsychENCODE project has identified over 2.5 million cis-eQTLs from 1387 samples of brain tissue from the prefrontal cortex, cerebellar cortex and thalamus cortex, gathered from related projects such as ENCODE, the CommonMind Consortium, GTEx and Roadmap.<citet data-cite="noauthor_comprehensive_nodate"><sup>psychencode</sup></citet>. The PsychENCODE project generated eQTLs using the standard GTEx pipeline<citet data-cite="noauthor_gtex_nodate-1"><sup>gtex_method</sup></citet> and the QTLtools package for eQTL identification, with a multiple testing correction made by limiting FDR values to less than 0.05.
#
# ##### eQTLGen
#
...
#
# This conclusion, that functional annotations augmented by positional data outperform positional-only annotations when using a metric of drugs significantly associated with a disease, compares favourably to the conclusions from the Gazal et al. study<citet data-cite="gazal_combining_2022"><sup>gazal</sup></citet> and OpenTargets dataset which it also analysed. The concluded optimal combined SNP-to-gene strategy, which the Gazal at al. study refers to as cS2G, was also shown to outperform any individual constituent strategy by combining several of them - the exon and promoter positional strategies along with several functional mappings such as fine-mapped eQTL, EpiMap and Activity-by-Contact (ABC). Similarly, the OpenTargets dataset was found to outperform a naive distance-based model with a combined functional and positional mapping strategy.
#
