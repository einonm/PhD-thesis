...
# The Fisher and Mann-Whitney p-values in table \ref{tab:ht_dti_comparison} also corroborate the conclusions made in the previous chapter - that both STITCH and DUGGIE are comparable and effective at detecting the difference between treatment and non-treatment hypertension drugs, as can be evidenced by the set of Fisher p-values, and DUGGIE is consistently but marginally better at differentiating between the treatment and non-treatment drug groups, as shown by the set of Mann-Whitney p-values.
...
#
# Table \ref{tab:hc_dti_comparison} arranges the results of the 32 gene set analysis pipeline executions for hypercholesteremia to briefly return to the discussion of the previous chapter, which concluded that DUGGIE has an advantage over STITCH in identifying significant drugs.
#
