...
# ## Other in-silico drug repurposing prediction methods

# The in-silico drug repurposing approaches discussed in section \ref{other-in-silico-methods-for-predicting-drug-repurposing-opportunities}, along with the gene-set analysis designed for this study can compliment each other in  efforts to gather evidence for repurposing approved drugs. Firstly, gene-set analysis (including the method employed in this study), interaction network analysis, TWAS and Connectivity Map signature matching can be used for a data-driven approach to identifying and ranking potential new drug repurposing opportunities.
#
# A gene set enrichment analysis, as a multi-target drug approach that can reflect more complex biology than single gene based analyses<citet data-cite="reay_advancing_2021"><sup>reay</sup></citet>, can also be used with gene pathways where pathways found to be associated with a disease can be used to identify repurposing opportunities from those drugs which are either known to affect the pathway or have known targets which interact with genes involved in the pathway. This enables drug targets acting at a greater number of pathway interaction steps away from disease genes to be identified, in contrast to this study's method which only involves drug targets that are also disease associated genes. However, pathway analysis does suffer from incomplete knowledge and inclusion of the pathway space. Also, gene set analysis in general is limited by lack of ability to map association signals at gene-dense loci without functional analysis or fine-mapping<citet data-cite="reay_advancing_2021"><sup>reay</sup></citet>, which this study attempts to mitigate by using functional mapping (in Chapter \ref{using-functional-qtl-gene-annotation-to-better-identify-genes-associated-with-disease}) and also testing the overlap of drug targets with the Major Histocompatibility Complex (MHC) region (see section \ref{major-histocompatibility-complex-mhc-considerations}).
#
# The direction of effect, where a gene set increases or decreases risk on a disease, is also something on which a gene set enrichment analysis does not inform. This is something that a TWAS can predict, and is also inherent to the Connectivity Map signature matching method. However, both of these methods are limited by the experimental evidence-backed data used - Connectivity Map signature matching by the limited cell lines and drugs for which perturbation responses are available<citet data-cite="zhao_decoding_2023"><sup>zhao23</sup></citet> and TWAS is prone to bias from non-trait related tissue data<citet data-cite="wainberg_opportunities_2019"><sup>wainberg</sup></citet>. 
#  
# TWAS also frequently identifies multiple hit genes per locus, and prioritises only candidate causal genes and tissues<citet data-cite="wainberg_opportunities_2019"><sup>wainberg</sup></citet>, without identifying causality. This is unlike Mendelian randomisation (MR) which can infer causal effects and can provide complimentary evidence on the identification of any new repurposing opportunities with a more hypothesis-driven approach, but involves complex considerations to select instrumental variabiles that satisfy the three MR assumptions stated in section \ref{other-in-silico-methods-for-predicting-drug-repurposing-opportunities}. Further, where MR involves proteins and pQTLs to infer drug-disease relationships, those pQTLs mainly involve whole blood tissue which is not often of relevance in complex diseases<citet data-cite="reay_advancing_2021"><sup>reay</sup></citet>. 
#
# The methods, data and results from these in-silico studies can aid in efforts to gather relevant evidence for repurposing drugs to treat new indications, such as NHS England's Medicines Repurposing programme<citet data-cite="noauthor_nhs_nodate"><sup>nhsrp</sup></citet> and the FDA CURE ID app<citet data-cite="research_cure_2020"><sup>cureid</sup></citet>, which aim to identify and collect proposals for candidate drug repurposing opportunities from crowdsourcing off-label use and facilitate the gathering of supporting evidence, direct new research and to aid licensing applications for the repurposed medicines.
...
#
# In addition, as an entirely in-silico study heavily involving p-values, no further experimental checks on the validity of results have been made, other than the comparison of known treatment drugs for hypertension and hypercholesterolemia and associated literature searches. Hence little consideration is made of confidence intervals or measuring any true effect size for the novel drugs indicated. This risk could be reduced by undertaking further supporting studies on the identified drugs, target gene sets and drug-disease relationships using the complimentary tools discussed in section \ref{other-in-silico-methods-for-predicting-drug-repurposing-opportunities}, such as TWAS and expression matching with Mendelian randomisation and colocalisation to interrogate the true causal loci involved in the disease-gene and drug-gene annotations used. Further target trial emulation and randomised control trials would be possible if enough evidence could be gathered promoting a particular drug-disease repurposing opportunity, and any evidence gathered provided to NHS England's Medicines Repurposing programme<citet data-cite="noauthor_nhs_nodate"><sup>nhsrp</sup></citet> and/or the FDA CURE ID app<citet data-cite="research_cure_2020"><sup>cureid</sup></citet> to support any initial off-target and licensing efforts.
#
# ## Concluding remarks
#
