...
# ## Drug target DB analysis - Removing genes found in STITCH
#
...
#
# We want the result to be the DB, but only with genes that are not present in STITCH
...
# temp - what targets does DUGGIE add to ...<ATC code>... over STITCH, and where do they come from?
display(
    "TTD"
    + str(list(jointall_df[jointall_df["ATC_TTD"] == "C08CA02"]["targets_entrez_TTD"]))
)
display(
    "DB"
    + str(
        list(
            jointall_df[jointall_df["ATC_TTD"] == "C08CA02"]["targets_entrez_DrugBank"]
        )
    )
)
display(
    "DGIdb"
    + str(
        list(jointall_df[jointall_df["ATC_TTD"] == "C08CA02"]["targets_entrez_DGIdb"])
    )
)
display(
    "STITCH"
    + str(
        list(jointall_df[jointall_df["ATC_TTD"] == "C08CA02"]["targets_entrez_STITCH"])
    )
)
display(
    "GtoPdb"
    + str(
        list(jointall_df[jointall_df["ATC_TTD"] == "C08CA02"]["targets_entrez_GtoPdb"])
    )
)
display(
    "T3DB"
    + str(list(jointall_df[jointall_df["ATC_TTD"] == "C08CA02"]["targets_entrez_T3DB"]))
)
display(
    "DsigDB"
    + str(
        list(jointall_df[jointall_df["ATC_TTD"] == "C08CA02"]["targets_entrez_DsigDB"])
    )
)
display(
    "DrugCentral"
    + str(
        list(
            jointall_df[jointall_df["ATC_DrugCentral"] == "C08CA02"][
                "targets_entrez_DrugCentral"
            ]
        )
    )
...
# ENSG00000001626 CFTR    1080    DGIdb, GtoPdb. DrugCentral
#
# ENSG00000012504 NR1H4   9971    DsigDB, DrugCentral
# ENSG00000042088 TDP1    55775   DsigDB
# ENSG00000053918 KCNQ1   3784    DsigDB
# ENSG00000091428 RAPGEF4 11069   DsigDB
# ENSG00000091831 ESR1    2099    DsigDB
# ENSG00000099331 MYO9B   4650    DGIdb
# ENSG00000102001 CACNA1F 778     DGIdb, DsigDB
# ENSG00000106546 AHR     196     DsigDB
# ENSG00000109320 NFKB1   4790    DsigDB
# ENSG00000110921 MVK     4598    DGIdb
# ENSG00000112312 GMNN    51053   DsigDB
# ENSG00000113580 NR3C1   2908    DsigDB
# ENSG00000116044 NFE2L2  4780    DsigDB
# ENSG00000121933 ADORA3  140     DsigDB, DrugCentral
# ENSG00000125384 PTGER2  5732    DsigDB
# ENSG00000135898 GPR55   9290    DsigDB
# ENSG00000138109 CYP2C9  1559    DsigDB. DrugCentral
# ENSG00000138356 AOX1    316     DrugCentral
# ENSG00000163930 BAP     8314    DsigDB
# ENSG00000164733 CTSB    1508    DsigDB
# ENSG00000173039 RELA    5970    DsigDB
# ENSG00000174600 CMKLR1  1240    DGIdb
# ENSG00000174697 LEP     3952    DGIdb
# ENSG00000185345 PRKN    5071    DsigDB
# ENSG00000186868 MAPT    4137    DsigDB
...
display(type(intermediate_atc_df["targets_entrez_STITCH"][5]))
display(intermediate_atc_df["targets_entrez_STITCH"])
...
final_single_df["all_targets_entrez_nostitch"] = ""
for index, row in final_single_df.iterrows():
    final_single_df.at[index, "all_targets_entrez_nostitch"] = row[
        "all_targets_entrez"
    ] - set(row["targets_entrez_STITCH"])
final_single_df.drop(["all_targets_entrez"], axis=1, inplace=True)
final_single_df.rename(
    columns={"all_targets_entrez_nostitch": "all_targets_entrez"}, inplace=True
)

final_combi_df["all_targets_entrez_nostitch"] = ""
for index, row in final_combi_df.iterrows():
    final_combi_df.at[index, "all_targets_entrez_nostitch"] = row[
        "all_targets_entrez"
    ] - set(row["targets_entrez_STITCH"])
final_combi_df.drop(["all_targets_entrez"], axis=1, inplace=True)
final_combi_df.rename(
    columns={"all_targets_entrez_nostitch": "all_targets_entrez"}, inplace=True
)

final_mix_df["all_targets_entrez_nostitch"] = ""
for index, row in final_mix_df.iterrows():
    final_mix_df.at[index, "all_targets_entrez_nostitch"] = row[
        "all_targets_entrez"
    ] - set(row["targets_entrez_STITCH"])
final_mix_df.drop(["all_targets_entrez"], axis=1, inplace=True)
final_mix_df.rename(
    columns={"all_targets_entrez_nostitch": "all_targets_entrez"}, inplace=True
)
...
conversion_df[conversion_df["Ensembl"] == "ENSG00000121933"]
...
final_df = final_df[final_df["atc_code"] != "nan"]
display(final_df)

final_df.to_csv(
    "../data/target-dbs/nostitch_dgi_targets_atc_ensembl.csv",
    sep="\t",
    header=None,
    index=None,
)

list(final_df[final_df["atc_code"] == "C08CA02"]["gene_list"])
