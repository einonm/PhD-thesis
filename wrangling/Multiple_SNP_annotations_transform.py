...
# ## Multiple SNP annotations - transform
#
...
df["log10_snpcount"].plot.bar()

df["snpcount"].plot.bar()

df2 = pd.read_csv("../data/wrangling/snp-to-genelist.annot.csv", sep="\t")

df2

groups = df2.groupby(["snp"])
snp_genelist_df = pd.DataFrame(columns=["snp", "genelist", "genecount"])

for name, group in groups:
    genelist = ""
    gene_count = 0
    for index, row in group.iterrows():
        genelist = genelist + " " + row["genelist"]
        gene_count = gene_count + 1
    new_row = {"snp": row["snp"], "genelist": genelist, "genecount": gene_count}
    snp_genelist_df.append(new_row, ignore_index=True)
